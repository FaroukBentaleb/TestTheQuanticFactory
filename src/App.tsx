    ...
    <>
      <div className="min-h-screen bg-gray-100 p-8">
        ...
        <div className="bg-white rounded-2xl shadow p-4 mb-6 flex flex-wrap gap-3 items-center">
          ...
          <label className="flex items-center gap-1 text-sm"><input type="radio" name="payant" checked={payant === ''} onChange={() => setPayant('')} /> Tous</label>
        ...
        {view === 'cards' && (
          <>
            ...
            <div className="grid grid-cols-3 gap-6">
              {data.map((item, index) => (
                <div key={`${item.source}-${item.nom}-${index}`} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 p-6 flex flex-col gap-3 border-t-4 border-green-500">
